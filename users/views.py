...
@login_required
def chat(request, myid):
	l1 = []
	to = Profile.objects.filter(id=myid)
	c = Chat.objects.all()

	l1.append(to[0].user.username)
	l1.append(request.user.username)
	...
	for i in c:
		print(type(i.u_to.user.username))
		if i.u_to.user.username in l1:
			print(i.u_from)
...
@login_required
def chatbox(request, myid):
	p = Profile.objects.filter(id=myid)
	if request.method == "POST":
		msg = request.POST['msg']

		ch = Chat(u_to=p[0], u_from=request.user, message=msg)
		ch.save()
	return redirect(f'/chat/{myid}/')