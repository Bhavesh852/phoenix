...
def addevent(request):
	if request.method == "POST":
		ename = request.POST['event']
		phone = request.POST['phone']
		sdate = request.POST['sdate']
		edate = request.POST['edate']
		des = request.POST['desc']

		e = Event(user=request.user, name=ename, phone=phone, start_date=sdate, expiry_date=edate, description=des)

		e.save()
		return redirect('/home/')	

@login_required
def viewevent(request, myid):
	event = Event.objects.filter(id=myid)
	return render(request, 'feed/viewevent.html', {
		'event' : event[0],
		})
...
def refresh(request):
	...
	din = t.split('-')
	for i in e:
		et = str(i.expiry_date)
		ev = et.split('-')
		if int(din[0])>int(ev[0]) or int(din[1])>int(ev[1]) or int(din[2])>int(ev[2]):
			i.delete()
	return redirect('/home/')
